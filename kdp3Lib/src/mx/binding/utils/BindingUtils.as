...
public class BindingUtils
{
                                ...
                                commitOnly:Boolean = false):ChangeWatcher
    {
        ...
        if (w != null)
        {
            ...
            {
            	//trace("bindProperty " + site + "." + prop + "=" + w.getValue());
            	//Solves the problem of entering null-values into properties
            	try {
                	site[prop] = w.getValue();
             	} catch(e:Error){
             		try {
						if (site.hasOwnProperty(prop) && !w.getValue())
						{
							//Strings cannot be equal to null
							site[prop] = site[prop] is String ? "" : null;
						}
	             	} catch(e:Error){
	             	}
             	}
