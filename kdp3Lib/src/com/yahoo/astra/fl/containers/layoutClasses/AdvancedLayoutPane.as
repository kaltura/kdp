...
package com.yahoo.astra.fl.containers.layoutClasses
{
	...
	//--------------------------------------
		...
		public function updateIncludeInLayout(target : UIComponent) : void
		{
			var configArr : Array = configuration.concat();
			for(var i : Number=0; i < configArr.length; i++)
			{
				if(configArr[i].target == target){
					configArr[i].includeInLayout = target.includeInLayout;
					break;
				}	
			}
			target.visible = target.includeInLayout;
		...
		override public function set enabled(value:Boolean):void
		{
			...
			{
				for each (var item:* in this.configuration)
				{
					if(item.target is UIComponent)
					{
						item.target.enabled = value;
					}
				}
			}
		}
