package com.kaltura.kdpfl.controller
{
	...
	public class AssignBehaviorCommand extends SimpleCommand
	{
		...
		override public function execute(note:INotification):void
		{
			...
			//if we have to support pre play sequence we need to create the command registration now
			if(sequenceProxy.vo.preSequenceCount > 0)
				facade.registerCommand( NotificationType.PRE_SEQUENCE_COMPLETE , PreSequenceEndCommand );
			
			//if we have to support post play sequence we need to create the command registration now
			if(sequenceProxy.vo.postSequenceCount > 0)
