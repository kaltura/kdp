package com.kaltura.kdpfl.controller
{
	...
	public class LoadConfigCommand extends AsyncCommand implements IResponder
	{
		...
		private function setLayout(xml:XML):void
		{
			...
			if (xml.descendants("Plugin").(attribute( "akamaiHD")).length() <= 0) {
				// add the following
//				var ak:XML = <Plugin id="akamaiHD" path="genericOSMFPlugin.swf" width="0%" height="0%" includeInLayout="false" loadingPolicy="onDemand" pluginURL="http://players.edgesuite.net/flash/plugins/osmf/advanced-streaming-plugin/fp10.0/staging/AkamaiAdvancedStreamingPlugin.swf" />;
				var ak:XML = <Plugin id="akamaiHD" path="genericOSMFPlugin.swf" width="0%" height="0%" includeInLayout="false" loadingPolicy="onDemand" pluginURL="AkamaiAdvancedStreamingPlugin.swf" />;
				xml.layout.VBox.(@id == "player")[0].prependChild(ak);
			}
