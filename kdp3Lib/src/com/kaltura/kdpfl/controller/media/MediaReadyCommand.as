		...
		override public function execute(notification:INotification):void
		{
			switch (notification.getName())
			{
				case NotificationType.MEDIA_READY:
					...
					if ((_mediaProxy.vo.entry is KalturaLiveStreamEntry && _mediaProxy.vo.isOffline) || (_flashvars.streamerType == StreamerType.LIVE && _mediaProxy.vo.isOffline) )
					{
						sendNotification(NotificationType.ENABLE_GUI, {guiEnabled : false , enableType : EnableType.CONTROLS});
						sendNotification(NotificationType.LIVE_ENTRY, _mediaProxy.vo.resource as URLResource);
					}
					//In each of these cases the media within the player needs to start playing immediately.
					else if(_mediaProxy.vo.singleAutoPlay || (_flashvars.autoPlay == "true") || _sequence.isInSequence)
					{
