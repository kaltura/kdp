package com.kaltura.kdpfl.view
{
	...
	public class CuePointsMediator extends Mediator
	{
		...
		override public function handleNotification(notification:INotification):void
		{
			...
			switch (notification.getName() )
			{
				...
				case NotificationType.CUE_POINTS_RECEIVED:
					...
					//TODO: implement a better solution to not trigger the AD_OPPORTUNITY before CUE_POINTS_RECEIVED
					setTimeout(findPrePostSequence,1);
					break;
		...
		public function addCuePoints(cpArray:Array):void {
			...
			for (var i:int = 0; i<cpArray.length; i++) {
				...
				if (cp.startTime==0 || cp.startTime==_entryDurationInMS) {
					...
					{
						sendNotification( NotificationType.AD_OPPORTUNITY ,{context : (cp.startTime==0) ? SequenceContextType.PRE : SequenceContextType.POST, cuePoint : cp} );	
					}
					else 
					{
						addToCPMap(cp);
		...
		protected function findPrePostSequence () : void
		{
			...
			if (_cuePointsMap[_entryDurationInMS] && _cuePointsMap[_entryDurationInMS].length)
			{
				postrollAdCuePoints = (_cuePointsMap[_entryDurationInMS] as Array).filter( isAdCuePoint );
				_cuePointsMap[_entryDurationInMS] = (_cuePointsMap[_entryDurationInMS] as Array).filter( isNotAdCuePoint );
			}
