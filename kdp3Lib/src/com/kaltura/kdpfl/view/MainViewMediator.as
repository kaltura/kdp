package com.kaltura.kdpfl.view
{
	...
	public class MainViewMediator extends Mediator
	{
		...
		override public function listNotificationInterests():Array
		{
		 ...
		 public function onIndexChange ( e: Event) : void
		{
			e.stopPropagation();
		}
				...
				(!item.hasOwnProperty('supportEnableGui') || 
					(item.hasOwnProperty('supportEnableGui') && item["supportEnableGui"] == "true" ) ) )
					{
						...
						if(!isEnabled)
						{
							...
							{
								if (!item.hasOwnProperty("excludeMe") && !(item is Plugin)) 
								{
									item.enabled = false;
								}
								
								if(item.hasOwnProperty("excludeMe") || item.hasOwnProperty("configuration"))
								{
									item.enabled=true;
								}
							}
