package com.kaltura.kdpfl.view.media
{
	...
	public class KMediaPlayerMediator extends Mediator
	{
		...
		{	
			...
			//New ABC requirement: if the flashvars specify a bitrate capping for the regular&fullscreen player, use them
		...
		override public function handleNotification(note:INotification):void
		{
			...
			switch(note.getName())
			{
				...
				case NotificationType.DO_PLAY: //when the player asked to play	
					...
					if(!sequenceProxy.vo.isInSequence && _mediaProxy.vo.entry.id && _mediaProxy.vo.entry.id!= "-1" && sequenceProxy.hasSequenceToPlay() )
					{
				...
				case NotificationType.DO_SEEK: //when the player asked to seek
					...
					if (player.state != MediaPlayerState.PAUSED && player.currentTime < player.duration)
					{
						_mediaProxy.vo.singleAutoPlay = true;
						_prevState = "playing";
					}
				...
				case NotificationType.HAS_OPENED_FULL_SCREEN:
					...
					//trace(player.maxAllowedDynamicStreamIndex);
					break;
		...
		{	
			...
			switch( event.state )
			{
				case MediaPlayerState.LOADING:
					...
					if ( !sequenceProxy.vo.isInSequence && _mediaProxy.vo.deliveryType == StreamerType.HDNETWORK)
					{
				...
				case MediaPlayerState.PLAYING: 
					...
					if (player.media != null && _mediaProxy.vo.mediaPlayFrom != -1 && !sequenceProxy.vo.isInSequence && _mediaProxy.vo.deliveryType != StreamerType.HDNETWORK)
					{	
					...
					if(player.isDynamicStream && !sequenceProxy.vo.isInSequence && !_doSwitchSent)
					{
		...
		private function onPlayHeadChange( event : TimeEvent ) : void
		{
			...
			if (player.temporal && !isNaN(event.time))
			{
				...
				if (sequenceProxy.vo.isInSequence)
				{
					...
					sequenceProxy.vo.timeRemaining = (!isNaN(event.time) && Math.round(duration - event.time) > 0) ? Math.round(duration - event.time) : 0;	
					//sequenceProxy.vo.timeRemaining = duration
				}
				if( !isNaN(event.time) )
				{
