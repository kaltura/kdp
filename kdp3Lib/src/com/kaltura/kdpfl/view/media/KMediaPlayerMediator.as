package com.kaltura.kdpfl.view.media
{
	...
	public class KMediaPlayerMediator extends Mediator
	{
		...
		{	
			...
			player.addEventListener( "connectionEnd", onConnectionEnd);
		...
		override public function handleNotification(note:INotification):void
		{
			...
			switch(note.getName())
			{
				...
				case NotificationType.DO_PLAY: //when the player asked to play	
					...
					if(_mediaProxy.vo.entry is KalturaLiveStreamEntry && _mediaProxy.vo.isOffline)
					{
						this._autoPlay = true;
					}
					else if(sequenceProxy.hasSequenceToPlay() )
					{
					...
					else if(player.canPlay) 
					{
						...
						if(_currentTime >= timeTrait.duration){
							sendNotification(NotificationType.DO_SEEK,0);
							player.addEventListener(TimeEvent.COMPLETE, onTimeComplete);
						}
						
						//if we did intelegent seek and reach the end of the movie we must load the new url
				...
				case LiveStreamCommand.LIVE_STREAM_READY: 
					...
					if(_autoPlay || _mediaProxy.vo.singleAutoPlay)
					{
						sendNotification(NotificationType.DO_PLAY);
					}
				break;
				...
				case NotificationType.DO_SEEK: //when the player asked to seek
					...
					/////////if the seek asked time is less than the loaded time///////
					////////////////TODO: if the video section is already loaded, make normal seek, not intelligent
					////////////////////// NOW WE CAN DO IT WITH THE NEW OSMF SO LET'S DO IT!! .....
		...
		public function setSource() : void
		{
			...
			if(player.state != MediaPlayerState.PLAYING)
			{
				...
				{
					_blockThumb = true;
					kMediaPlayer.hideThumbnail(); //hide the thumnail	
				}
				else
				{
					_blockThumb = false;
					if(!_isIntelliSeeking )//if if seeking, don't show the thumbnail
						kMediaPlayer.showThumbnail(); //show the thumnail	
				}
			}
			...
			switch( event.state )
			{
				case MediaPlayerState.READY: 
					...
					if(!_playerReadyOrEmptyFlag)
					{
						...
						//Today when the player is ready the kdp is ready to play, tommorow we can deside that
						//somthing else need to happen for the KDP to be ready so we have 2 events that we send now
						//but again tommorow the next event can be moved to other place and depend on other things
				...
				case MediaPlayerState.PAUSED:
				     	//If the player is playing a live stream entry, and is paused, the connectionEnd mechanism needs to pause
			     	if(_mediaProxy.vo.entry is KalturaLiveStreamEntry)
					{
						if(_postLive)
						{
							if(_postLive.hasEventListener("connectionEnd"))
							{
								_postLive.removeEventListener("connectionEnd", onConnectionEnd);
								if(_postLive) _postLive.pauseTimer();
							}
						}
					}
				...
				case MediaPlayerState.PLAYING: 
					...
					//In a live stream entry, when play is commenced it is necessary to begin a mechanism testing whether the connection is down.
					if(_mediaProxy.vo.entry is KalturaLiveStreamEntry)
					{
						if(!_postLive){
							_postLive = new PostLiveStreamAssist(facade);
						}
						else
						{
							_postLive.startTimer();
						}
						_postLive.addEventListener("connectionEnd", onConnectionEnd ) ;
					}
					sendNotification( NotificationType.PLAYER_PLAYED );
				break;
		...
		{
			_intelliSeekStart = _duration - event.time;
		...
		private function onSwitchingChange( event : DynamicStreamEvent ) : void
		{
			...
			sendNotification( NotificationType.SWITCHING_CHANGE );
		}
		...
		private function onConnectionEnd ( e: Event): void
		{
			_mediaProxy.vo.isOffline = true;
			_mediaProxy.vo.singleAutoPlay = true;
			var postSequence : SequenceProxy = facade.retrieveProxy(SequenceProxy.NAME) as SequenceProxy;
			sendNotification(NotificationType.DO_PAUSE);
			sendNotification(NotificationType.ENABLE_GUI, {guiEnabled : false , enableType : EnableType.CONTROLS});
			if(postSequence.vo.postSequenceCount > 0)
			{
				sendNotification(NotificationType.POST_1_START);
			}else{
				sendNotification(NotificationType.CHANGE_MEDIA, {entryId: _mediaProxy.vo.entry.id, flavorId: null, preferedFlavorBR: null });
			}
		}
	}
}