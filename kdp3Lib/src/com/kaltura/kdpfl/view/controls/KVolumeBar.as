...
package com.kaltura.kdpfl.view.controls
{
	...
	public dynamic class KVolumeBar extends UIComponent implements IComponent
	{
		...
		// TODO update comment
		/**
		 * Dispatched when there's a volume change
		 *
		 * @langversion 3.0
		...
		/**
		 * Dispatched when the volume change was completed
		 *
		 * @langversion 3.0
		 * @playerversion Flash 9.0.28.0
		 */
		static public const EVENT_CHANGE_END:String = "eventChangeEnd";
		...
		/**
		 * set an initial value to the volume bar. 0 to 1
		 */
		public var initialValue:Number;
		
		/**
		 * if this is set to true - this value will override the flash cookie 
		 */
		...
		/**
		 * adds the volumeSlider, only if it isn't a child of sliderContainer yet
		 * */
		private function onAdded (e : Event = null) : void
		{		
			if(sliderContainer && _volumeSlider && (!sliderContainer.contains(_volumeSlider)))
			{
		 ...
		 * @param volume
		 * @param dispatchEventChange optional, if true will send change event
		 * 
		 */	
		public function changeVolume( volume:Number, dispatchEventChange:Boolean = true ):void
		{
			setVolume( volume );
		 ...
		 * 
		 */	
		private function onSliderClick (e:Event) : void
		{
		...
		/**
		 * show/hide the volume button according to given value
		 * */
		public function showVolumeButton(value:Boolean):void {
			_showButton = value;
			if (_button)
				_button.visible = _showButton;
		}
