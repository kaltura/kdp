package com.kaltura.kdpfl.model
{
	...
	public class LayoutProxy extends Proxy
	{
		...
		/**
		 * indicates number of plugins in loading process 
		 */		
		private var _pluginsCounter:int = 0;
		
		/**
		 * Map of plugins, key=plugin id, value = pluginReady/pluginNotReady 
		 */		
		...
		public function buildLayout( xml:XML , itemRendererData : Object = null):Object
		{
			...
			if(xml.name() == "Plugin")
			{
				...
				pluginsMap[xml.@id] = PluginStatus.NOT_READY;
				...
				(uiComponent as Plugin).addEventListener( IOErrorEvent.IO_ERROR , onPluginError );
				(uiComponent as Plugin).addEventListener( SecurityErrorEvent.SECURITY_ERROR , onPluginError );
				(uiComponent as Plugin).addEventListener( ErrorEvent.ERROR , onPluginError );
				(uiComponent as Plugin).addEventListener( AsyncErrorEvent.ASYNC_ERROR , onPluginError );
...
/* 						 	else //the configuration replace the need to add child
		...
		private function onPluginReady( event : Event ) : void
		{
			...
			pluginsMap[plugin.name] = PluginStatus.READY;		
		...
		private function onPluginError( event: Event) : void 
		{
			var plugin : Plugin = ( event.target as Plugin );
			removePluginListeners(plugin);
			pluginsMap[plugin.name] = PluginStatus.LOAD_ERROR;	
         ...
         public function FindCompByName (compName : String) : Object
        {
		 ...
		 private function checkAllPluginsLoaded () : void {
			 if (_pluginsCounter==0) 
			 	sendNotification(NotificationType.PLUGINS_LOADED, pluginsMap);
		 }
	}
}
