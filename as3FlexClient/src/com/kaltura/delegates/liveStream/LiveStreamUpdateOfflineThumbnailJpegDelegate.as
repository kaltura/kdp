...
package com.kaltura.delegates.liveStream
{
	...
	public class LiveStreamUpdateOfflineThumbnailJpegDelegate extends WebDelegateBase
	{
		...
		public function LiveStreamUpdateOfflineThumbnailJpegDelegate(call:KalturaCall, config:KalturaConfig)
		{
			super(call, config);
		}

		override public function parse(result:XML):* {
			if ((call as LiveStreamUpdateOfflineThumbnailJpeg).fileData is FileReference) {
		...
		override protected function sendRequest():void {
			...
			if ((call as LiveStreamUpdateOfflineThumbnailJpeg).fileData is FileReference) {
				(call as LiveStreamUpdateOfflineThumbnailJpeg).fileData.addEventListener(DataEvent.UPLOAD_COMPLETE_DATA,onDataComplete);
				var urlRequest:URLRequest = new URLRequest(req);
				((call as LiveStreamUpdateOfflineThumbnailJpeg).fileData as FileReference).upload(urlRequest,"fileData");
			}
			else{
				mrloader.addFile(((call as LiveStreamUpdateOfflineThumbnailJpeg).fileData as ByteArray), UIDUtil.createUID(), 'fileData');	
