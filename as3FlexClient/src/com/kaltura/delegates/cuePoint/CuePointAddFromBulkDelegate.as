...
package com.kaltura.delegates.cuePoint
{
	...
	public class CuePointAddFromBulkDelegate extends WebDelegateBase
	{
		protected var mrloader:MultipartURLLoader;

		public function CuePointAddFromBulkDelegate(call:KalturaCall, config:KalturaConfig)
		{
			super(call, config);
		}

		override public function parse(result:XML):* {
			if ((call as CuePointAddFromBulk).fileData is FileReference) {
		...
		override protected function sendRequest():void {
			...
			if ((call as CuePointAddFromBulk).fileData is FileReference) {
				(call as CuePointAddFromBulk).fileData.addEventListener(DataEvent.UPLOAD_COMPLETE_DATA,onDataComplete);
				var urlRequest:URLRequest = new URLRequest(req);
				((call as CuePointAddFromBulk).fileData as FileReference).upload(urlRequest,"fileData");
			}
			else{
				mrloader.addFile(((call as CuePointAddFromBulk).fileData as ByteArray), UIDUtil.createUID(), 'fileData');	
