...
package com.kaltura.delegates.categoryUser
{
	...
	public class CategoryUserAddFromBulkUploadDelegate extends WebDelegateBase
	{
		protected var mrloader:MultipartURLLoader;

		public function CategoryUserAddFromBulkUploadDelegate(call:KalturaCall, config:KalturaConfig)
		{
			super(call, config);
		}

		override public function parse(result:XML):* {
			if ((call as CategoryUserAddFromBulkUpload).fileData is FileReference) {
		...
		override protected function sendRequest():void {
			...
			if ((call as CategoryUserAddFromBulkUpload).fileData is FileReference) {
				(call as CategoryUserAddFromBulkUpload).fileData.addEventListener(DataEvent.UPLOAD_COMPLETE_DATA,onDataComplete);
				var urlRequest:URLRequest = new URLRequest(req);
				((call as CategoryUserAddFromBulkUpload).fileData as FileReference).upload(urlRequest,"fileData");
			}
			else{
				mrloader.addFile(((call as CategoryUserAddFromBulkUpload).fileData as ByteArray), UIDUtil.createUID(), 'fileData');	
