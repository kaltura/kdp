...
package com.kaltura.delegates.user
{
	...
	public class UserAddFromBulkUploadDelegate extends WebDelegateBase
	{
		protected var mrloader:MultipartURLLoader;

		public function UserAddFromBulkUploadDelegate(call:KalturaCall, config:KalturaConfig)
		{
			super(call, config);
		}

		override public function parse(result:XML):* {
			if ((call as UserAddFromBulkUpload).fileData is FileReference) {
		...
		override protected function sendRequest():void {
			...
			if ((call as UserAddFromBulkUpload).fileData is FileReference) {
				(call as UserAddFromBulkUpload).fileData.addEventListener(DataEvent.UPLOAD_COMPLETE_DATA,onDataComplete);
				var urlRequest:URLRequest = new URLRequest(req);
				((call as UserAddFromBulkUpload).fileData as FileReference).upload(urlRequest,"fileData");
			}
			else{
				mrloader.addFile(((call as UserAddFromBulkUpload).fileData as ByteArray), UIDUtil.createUID(), 'fileData');	
