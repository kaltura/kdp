...
package com.kaltura.plugin.logic.properties.restrictions
{
	 ...
	 */	
	public class RestrictionNumber extends BaseRestriction
	{
		public static const TYPE:String = "number"
		/**
		* The minimum allowed value.
		*/		
		protected var _nMinLimit:Number;
		/**
		* The maximum allowed value.
		*/		
		protected var _nMaxLimit:Number;
		/**
		* Non-zero unit of change between values.
		 * @default 1
		*/		
		protected var _nStepSize:Number = 1;
		
		/**
		* Indicates whether a range restriction is set
		*/		
		protected var _bRangeRestriction:Boolean = false;
		...
		{
			var nDataToCheck:Number = Number(dataToCheck);
		...
		public override function setRestriction(xmlRestriction:XML):void
		{
			...
			if (xmlRestriction.range.length()){
				...
				if (xmlRestriction.range.min.length()){
					_nMinLimit = Number(xmlRestriction.range.min.@value);
				} else {
					_nMinLimit = -Number.MAX_VALUE;
				}
				if (xmlRestriction.range.max.length()){
					_nMaxLimit = Number(xmlRestriction.range.max.@value);
				} else {
					_nMaxLimit = Number.MAX_VALUE;
				}
