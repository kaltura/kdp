	...
	public class ManifestParser
	{
		...
		{			
			...
			if (value.media.length == 1)  // Single Stream/Progressive Resource
			{
				...
				{
					var baseUrlWithNoParams:Array = baseURLString.split("?"); 
					...
					resource = new StreamingURLResource(baseUrlWithNoParams[0] + "/" + url, streamType(value),NaN, NaN, null, numAppInstances);
				}
			...
			else if (value.media.length > 1) // Dynamic Streaming
			{
				...
				if (baseURL)
				{
					var urlWithNoParams:Array = baseURL.split("?"); 
					var splittedBaseUrl : Array = urlWithNoParams[0].split("/");
					numAppInstances = splittedBaseUrl.length - 4 > 0 ? splittedBaseUrl.length - 4 : 0;
				}
				serverBaseURLs = new Vector.<String>();
