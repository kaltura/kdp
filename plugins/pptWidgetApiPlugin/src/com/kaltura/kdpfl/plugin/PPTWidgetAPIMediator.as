package com.kaltura.kdpfl.plugin
{
	...
	public class PPTWidgetAPIMediator extends Mediator
	{
		...
		protected function slideLoadComplete(event:Event):void
		{
			...
			var errorTimer : Timer = new Timer (2000,1);
			errorTimer.addEventListener(TimerEvent.TIMER_COMPLETE, onErrorTimerComplete);
			errorTimer.start();
		...
		protected function onErrorTimerComplete ( e : TimerEvent ) : void
		{
			(e.target as Timer).stop();
			(e.target as Timer).removeEventListener(TimerEvent.TIMER_COMPLETE, onErrorTimerComplete );
			getSlidePictures();
		}
		...
		protected function getSlidePictures():void 
		{
			...
			for (var index:int = 1; index <= numberOfSlidesInCarousel; index++) 
			{
