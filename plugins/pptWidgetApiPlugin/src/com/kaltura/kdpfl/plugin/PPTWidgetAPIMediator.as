package com.kaltura.kdpfl.plugin {
	...
	public class PPTWidgetAPIMediator extends Mediator {
		...
		override public function onRegister():void {
			...
			_debugMode = _flashvars.debugMode == "true";
		...
		override public function handleNotification(note:INotification):void {
			switch (note.getName()) {
				...
				case "videoMarkHighlight":
					(viewComponent as PPTWidgetAPIPluginCode).isMarkSelected = true;
					break;
				...
				case PPTWidgetNotifications.SLIDES_LOADED:
					gotoSlide((viewComponent as PPTWidgetAPIPluginCode).currentFrame);
					break;
			}
		}
		...
		protected function slideLoadComplete(event:Event):void {
			...
			(viewComponent as PPTWidgetAPIPluginCode).presentationMovieClip = _slideLoader.content as MovieClip;

			if (!(viewComponent as PPTWidgetAPIPluginCode).adminMode) {
				...
				for (var index:int = 1; index <= (viewComponent as PPTWidgetAPIPluginCode).presentationMovieClip.totalFrames; index++) {
					dataProvider.addItem({frameNum: index, source: _slideLoader.content as MovieClip});
				}
			}

			(viewComponent as PPTWidgetAPIPluginCode).carouselDataProvider = dataProvider;

			if (dataProvider && dataProvider.length)
				(viewComponent as PPTWidgetAPIPluginCode).currentFrame = dataProvider.getItemAt(0)["frameNum"];
		...
		private function gotoSlide(slideIndex:int):void {
			viewComponent.currentFrame = slideIndex;
		...
		protected function onPPTWidgetNextCarouselSlide():void {
			...
			gotoSlide(viewComponent.currentFrame + 1);
		}
		...
		protected function onPPTWidgetAddMark(note:INotification):void {
			...
			var newMark:Object = {video: videoTime, slide: (viewComponent as PPTWidgetAPIPluginCode).currentFrame};
		...
		protected function onPPTWidgetRemoveMark(note:INotification):void {
			...
			(viewComponent as PPTWidgetAPIPluginCode).isMarkSelected = false;
			shouldSave = true;
		}
		...
		protected function saveDataEntry():void {
			...
			entry.retrieveDataContentByGet = (viewComponent as PPTWidgetAPIPluginCode).dataEntry.retrieveDataContentByGet;
