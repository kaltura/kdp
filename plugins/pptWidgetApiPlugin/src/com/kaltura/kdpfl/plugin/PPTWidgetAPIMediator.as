package com.kaltura.kdpfl.plugin
{
	...
	public class PPTWidgetAPIMediator extends Mediator
	{
		...
		override public function handleNotification(note:INotification):void
		{
			switch(note.getName())
			{
				...
				case NotificationType.CHANGE_MEDIA_PROCESS_STARTED:
					...
					break;
				case SLIDES_LOADED :
					gotoSlide((viewComponent as PPTWidgetAPIPlugin).currentFrame);	
					break;
			}
		}
		...
		protected function slideLoadComplete(event:Event):void
		{
			...
			(viewComponent as PPTWidgetAPIPlugin).presentationMovieClip = _slideLoader.content as MovieClip;
			
			if (!(viewComponent as PPTWidgetAPIPlugin).adminMode)
			{
			...
			{
				for (var index:int =1; index<= (viewComponent as PPTWidgetAPIPlugin).presentationMovieClip.totalFrames; index++)
				{
					dataProvider.addItem( { frameNum : index, source: _slideLoader.content as MovieClip} );
				}
			}
			
			(viewComponent as PPTWidgetAPIPlugin).carouselDataProvider = dataProvider;
			
			(viewComponent as PPTWidgetAPIPlugin).currentFrame = dataProvider.getItemAt(0)["frameNum"];
		...
		protected function onPPTWidgetAddMark(note:INotification):void
		{
			...
			var newMark : Object = { video: videoTime, slide: (viewComponent as PPTWidgetAPIPlugin).currentFrame };
