...
package com.kaltura.kdpfl.plugin.widevine
{
	...
	import com.kaltura.kdpfl.plugin.widevine.WvSocket;
	import com.kaltura.kdpfl.plugin.widevine.WvChapter;
	
	class WvChapterConnection // extends EventDispatcher 
	{
		// various chapter states
		const CHAPTER_IDLE:int					= 0;
		const CHAPTER_CONNECTING:int 			= 1;
		const CHAPTER_CONNECTED:int 			= 2;
		const CHAPTER_QUERY_LOADED_STATUS_LOADING:int = 3;
		const CHAPTER_QUERY_LOADED_STATUS_LOADED:int = 4;
		const CHAPTER_NUM_CHAPTERS_LOADING:int 	= 5;
		const CHAPTER_NUM_CHAPTERS_LOADED:int 	= 6;
		const CHAPTER_DATA_LOADING:int 			= 7;
		const CHAPTER_DATA_LOADED:int 			= 8;	
		const CHAPTER_IMAGE_LOADING:int 		= 9;
		const CHAPTER_IMAGE_LOADED:int 			= 10;		
		const MAX_RETRIES_ALLOWED:int 			= 11;
		...
		public function closeSocket():void
		{
			...
			var i:int = 0;
			for each (var chapter in myChapters) {
				chapter.close();
				delete myChapters[i++];
			}
			myChapters = null;
