package com.kaltura.kdpfl.plugin.component
{
	...
	public class ClosedCaptions extends KHBox //implements IComponent
	{
		...
		public function loadCaptions (fileUrl:String, fileType:String):void
		{
			...
			function parseTimedText (e:Event):void
			{
				try{
					...
					{
						stylingObject[style.@id[0].toString()] = {tf: xmlToTextFormat(style), backgroundColor:xmlToBGColor(style)};	
					}
					...
					for (var i:int = 0; i < numOfLines; i++)
					{
						...
						{
							currLine.textFormat = new TextFormat (fontFamily, _fontSize, 0xffffff, null, null, null, null, null, "center")
						}
		...
		private function xmlToTextFormat (style : XML) : TextFormat
		{
			...
			tf.font = style.@xmlns_tts::fontFamily.length() ? style.@xmlns_tts::fontFamily[0].toString() : fontFamily;
			tf.size = style.@xmlns_tts::fontSize.length() ? style.@xmlns_tts::fontSize[0].toString() : _fontSize;
		...
		public function updatePlayhead (pos:Number):void
		{
			...
			for (var i:int = 0; i < _currentCCFile.length; i++)
			{
				...
				if (pos <= line.end)
				{
					if (pos >= line.start)
					{
						setText (line.text, line.textFormat, line.backgroundColor);
					}
					else
					{
						setText ("");
		...
		public function setText (text:String, textFormat : TextFormat = null, bgColor:Number=0):void
		{
			var tf : TextFormat;
			if (textFormat)
			{
