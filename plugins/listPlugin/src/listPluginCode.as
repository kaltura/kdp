...
package
{
	...
	public class listPluginCode extends UIComponent implements IPlugin
	{
		...
		public function listPluginCode()
		{
			...
			_klist.addEventListener( ListEvent.ITEM_CLICK, onListItemClick, false, 0, true );
	...
	//		_klist.itemContentFactory = samplePlaylistItemFactory; 
		...
		public function onListItemClick( evt:ListEvent ):void
		{
			...
			//we wont get a change event if it's the same index but we want to play it
			if( _dataProvider.selectedIndex==evt.index && _dataProvider == evt.target.dataProvider )
			{
			   _facade.sendNotification("doStop");
			   _facade.sendNotification("doPlay");
			}
		}
	
		// TODO remove
	/* 	public function samplePlaylistItemFactory( layout:XML, data:Object ):UIComponent
		{
			return( new SamplePlaylistItem );
		}	 */
