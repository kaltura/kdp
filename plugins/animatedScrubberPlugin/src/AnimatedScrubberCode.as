...
package  
{
	 ...
	 * 
	 */
	public dynamic class AnimatedScrubberCode extends UIComponent implements IComponent, IPlugin
	{			
		...
		[Bindable]
		public var color7:Number=-1;//change colors to all instance ids with "dynamicColor" as part of name.  
		
		public var thumbYOffset:Number	=2;
		
		public var thumbXOffset:Number	= 0;
		...
		//asthetic treatment for scrubber track.  
		public var scrubberDecoration:String;
		public var scrubberMask:String;
		...
		protected static var defaultStyles:Object =
			{
				...
				thumbUpSkin: "Scrubber_thumbUp",
				thumbOverSkin: "Scrubber_thumbOver",
				thumbDownSkin: "Scrubber_thumbDown",
				thumbDisabledSkin: "Scrubber_thumbDisabled",
				focusRectSkin:null,
		 ...
		 * @playerversion Flash 9.0.28.0
		 */
		public function AnimatedScrubberCode()
		{
			super();
		}
		...
		public function initializePlugin( facade : IFacade ) : void{
			var mediator:AnimatedScrubberMediator	= new AnimatedScrubberMediator(this);
				facade.registerMediator(mediator);
			
			if(tooltip == "00:00")//indicate that tooltip gets updated with time at mouse x position.
		...
		private function onMouseMove(e:MouseEvent):void{
			var relativePercentage:Number	= (mouseX/this.width);
			var localTime:Number			= Math.ceil(relativePercentage*duration);
			if(e.target.name != "grip"){
		...
		override protected function draw():void
		{
			...
			if(_trackProgMask && _trackLoadMask)
				resizeMask();
			
			if(_treatment)
				resizeTreatment();
			
			if(_thumb.x < thumbXOffset && thumbXOffset != 0)//default thumb placement
				_thumb.x 				= thumbXOffset;
			
			
			validate(); // because we're not calling super.draw		
		}
		
		private var _trackProgMask:Sprite;
		private var _trackLoadMask:Sprite;
		private var _treatment:Sprite;
		
		private function resizeMask():void{
			_trackProgMask.getChildAt(0).width	= _durationIndicator.width;
			_trackLoadMask.getChildAt(0).width	= _durationIndicator.width;
			_trackProgMask.height	= _trackLoadMask.height = _durationIndicator.height;
			_trackProgMask.x		= _trackLoadMask.x = _durationIndicator.x;
			_trackProgMask.y		= _trackLoadMask.y = _durationIndicator.y;
		}
		
		private function resizeTreatment():void{
			_treatment.width	= _durationIndicator.width+2;
			_treatment.height	= _durationIndicator.height;
			_treatment.x		= _durationIndicator.x;
			_treatment.y		= _durationIndicator.y;			
		}
		
		private function drawMask():void{
			if(scrubberMask == null)
				return;

			var ClassMask:Class	= getDefinitionByName(scrubberMask) as Class;
			_trackLoadMask			= new ClassMask as Sprite;
			_trackProgMask			= new ClassMask as Sprite;
			
			_trackLoadMask.getChildAt(0).width	= _durationIndicator.width;
			_trackLoadMask.height	= _durationIndicator.height;
			_trackLoadMask.cacheAsBitmap	= true;
			
			_trackProgMask.getChildAt(0).width	= _durationIndicator.width;
			_trackProgMask.height	= _durationIndicator.height;
			
			_trackProgMask.cacheAsBitmap	= true;
			_trackLoadMask.cacheAsBitmap	= true;
			
			_loadIndicator.cacheAsBitmap		= true;
			_loadIndicator.mask		= _trackLoadMask;
			_progressIndicator.cacheAsBitmap	= true;
			_progressIndicator.mask	= _trackProgMask;
			
			addChild(_trackLoadMask);
			addChild(_trackProgMask);
		}

		private function drawTreatment():void{
			if(_treatment || scrubberDecoration == null)
				return;
			
			var ClassTreatment:Class	= getDefinitionByName("ScrubberGradient") as Class;
			_treatment					= new ClassTreatment as Sprite;
			_treatment.width	= _durationIndicator.width;
			_treatment.height	= _durationIndicator.height;

			addChildAt(_treatment,this.numChildren-2);
		}
		...
		private var _thumbHeight:Number	= -1;
		private var _thumbWidth:Number	= -1;
...
//OVER HERE
			...
			//_thumb.y = (25 - _thumb.height)/2;
			_thumb.y		= _loadIndicator.y+thumbYOffset; 
		...
		public function setPosition( value:Number ):void
		{
			...
			if(!_scrubbing && _duration && _thumb) 
			{
				...
				if(! isNaN( value/_duration * _range ) )
				{
					...
					if(thumbXOffset != 0)
						_thumb.x = (p * (width-thumbXOffset*2))+thumbXOffset;
					else 
						_thumb.x = (p * width);
				...
				{
					_thumb.x  = (_progressIndicator.width = 0)+_thumb.width;
				}
		...
		protected function scrubButtonMouseOver(event:MouseEvent):void{
			Tweener.addTween(event.target, {width:this._thumbWidth+4, height:this._thumbHeight+4, time:.25, transition:"easeOutQuart"});
		}
		
		protected function scrubButtonMouseOut(event:MouseEvent):void{
			Tweener.addTween(event.target, {width:this._thumbWidth, height:this._thumbHeight, time:.25, transition:"easeOutQuart"});
		}
		...
		protected function scrubButtonMouseDownHandler(event:MouseEvent):void
		{
			...
			_isDragging = true;
			var evtDragStart:Event = new Event( EVENT_DRAG, true, true );
			this.dispatchEvent( evtDragStart );	
			var evt:Event = new Event( EVENT_SEEK_START, true, true );
			this.dispatchEvent( evt );
			_thumb.x = Math.max(0, Math.min(_range, mouseX));

			if(thumbXOffset > 0)
				_progressIndicator.width = _thumb.x;
			else
				_progressIndicator.width = _thumb.x + _thumb.width/2;
			...
			evt = new Event( EVENT_SEEK, true, true );
			this.dispatchEvent( evt );
		...
		protected function scrubButtonMouseUpHandler(event:MouseEvent):void
		{
			...
			{
				_isDragging = false;
				var evtStopDrag:Event = new Event( EVENT_DRAG_END, true, true );
			...
			if( !liveScrubbing )
			{
				var evt:Event = new Event( EVENT_SEEK, true, true );
				this.dispatchEvent( evt );
			}
			evt = new Event( EVENT_SEEK_END, true, true );
			this.dispatchEvent( evt );
		}
		...
		protected function scrubButtonMouseMoveHandler(event:MouseEvent):void
		{
			...
			//make sure thumb stays in track
			
			if(_thumb.x+thumbXOffset < 0 && thumbXOffset != 0)
				_thumb.x = _thumb.x+thumbXOffset;
			
			if(_thumb.x+thumbXOffset > _durationIndicator.x+_durationIndicator.width  && thumbXOffset != 0)
				_thumb.x = _durationIndicator.x+_durationIndicator.width-thumbXOffset;
		...
		public function set allowMouseClicks(value:String):void
		{
			...
			else
			{
				...
				if (_thumb.hasEventListener(MouseEvent.MOUSE_DOWN) )
					...
					_thumb.removeEventListener(MouseEvent.MOUSE_DOWN, scrubButtonMouseOver);
