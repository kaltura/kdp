package com.kaltura.kdpfl.plugin.component
{
	...
	public class NielsenMediator extends Mediator
	{
		...
		override public function handleNotification(note:INotification):void
		{
			...
			switch(note.getName())
			{
				...
				case NotificationType.PLAYER_PAUSED:
					...
					{
						ggCom.getInstance().PM(6, _sendPos);
				...
				case NotificationType.PLAYER_UPDATE_PLAYHEAD:
					if (_sequenceProxy.vo.isInSequence)
						...
						_playheadPosition = Number(_playheadPosition.toFixed(1));
					}
				...
				case NotificationType.PLAYER_PLAYED:
					if (_lastState != 'playerPlayed')
					{
					//	ggCom.getInstance().PM(5,_sendPos);
						if(_lastStateAds == 'adStart')
						{
							_lastStateAds = 'adPlay';
							ggCom.getInstance().PM(15, _adUrl, _sequenceProxy.sequenceContext + "roll", this.getAdInfoFromMetadata(), this.getSegmentFromCuePoints());
						...
						else if (!_sequenceProxy.vo.isInSequence)
						{
							...
							{
								ggCom.getInstance().PM(15, (_mediaProxy.vo.resource as URLResource).url, 'content', this.getVideoInfoFromMetadata(), _seg);
				...
				case AdsNotificationTypes.AD_START:
					...
					if(_adType != "mainroll")
					{
						if(_lastStateMedia == 'mediaInfoSent')
						{
							...
							ggCom.getInstance().PM(7, this._sendPos);
							ggCom.getInstance().PM(4, this._sendPos, "content");
				...
				case AdsNotificationTypes.AD_CLICK:
					if(_lastStateAds == 'adPlay')
						ggCom.getInstance( ).PM( 16, _playheadPosition);
					break;
				...
				case AdsNotificationTypes.AD_END:
					...
					ggCom.getInstance().PM(4, _adPlayhead, "ad");
					_lastStateAds = 'adEnd';
					var noteBody:Object = note.getBody();
					for(var i:String in noteBody)
						trace(i +" =adEnd note.body= "+noteBody[i]);
					/*if(_adType != "postroll" && (_lastStateMedia == 'unloaded' || _lastStateMedia == 'mediaReady'))
					{
						ggCom.getInstance().PM(15, (_mediaProxy.vo.resource as URLResource).url, 'content', this.getVideoInfoFromMetadata(), this.getSegmentFromCuePoints());
						_lastStateMedia = 'mediaInfoSent';
						_lastState = "playerPlayed";
		...
		private function sendAdStop():void
		{
			...
			ggCom.getInstance().PM(4, _adPlayhead, "ad");
			_lastStateAds = 'adEnd';
			/*if(_adType != "postroll" && _lastStateMedia == 'unloaded')
			{
				ggCom.getInstance().PM(15, (_mediaProxy.vo.resource as URLResource).url, 'content', this.getVideoInfoFromMetadata(), this.getSegmentFromCuePoints());
				_lastStateMedia = 'mediaInfoSent';
			}*/
		...
		private function getVideoInfoFromMetadata():String
		{
			...
			_duration = isNaN(metadata.duration) ? _duration : metadata.duration;
		...
		private function getAdInfoFromMetadata():String
		{
			...
			var xml:XML = new XML("<videoInfo>"+this.getVideoInfoFromMetadata()+"</videoInfo>");
			var tag:String = "length";
			...
			xml.hasOwnProperty(tag) ? xml.replace(tag,<{tag}>{_durationAds.toFixed(2)}</{tag}>) : xml.appendChild(<{tag}>{_durationAds.toFixed(2)}</{tag}>);
