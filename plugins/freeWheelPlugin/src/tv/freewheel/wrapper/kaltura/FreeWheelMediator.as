package tv.freewheel.wrapper.kaltura
{
	...
	public class FreeWheelMediator extends SequenceMultiMediator 
	{
		...
		public static const UPDATE_VIDEO_ASSET_DURATION:String = "updateVideoAssetDuration";
		...
		override public function handleNotification(notification:INotification):void{
			...
			switch (notification.getName()){
				...
				case UPDATE_VIDEO_ASSET_ID:
					...
					//in this case we won't get the ENTRY_READY notification, so we should call notifyEntryReady now
					if (configProxy.vo.flashvars.sourceType!=SourceType.ENTRY_ID)
					{
						this.plugin.notifyEntryReady();
					}
					break;
				
				case UPDATE_VIDEO_ASSET_DURATION:
					this.plugin.currentAssetDuration = notification.getBody().videoAssetDuration;
					//in this case we won't get the ENTRY_READY notification, so we should call notifyEntryReady now
					if (configProxy.vo.flashvars.sourceType!=SourceType.ENTRY_ID)
					{
						this.plugin.notifyEntryReady();
		...
		{
			return this.facade.retrieveProxy(ConfigProxy.NAME) as ConfigProxy;
		}
	}
}
