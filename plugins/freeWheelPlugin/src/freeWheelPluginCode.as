package
{
	...
	public class freeWheelPluginCode extends Sprite implements IPlugin, ISequencePlugin, IMidrollSequencePlugin
	{
		...
		/**
		 *  
		 * duration of current entry. To be used when we don't have entry object, i.e when sourceType!=entryId 
		 * 
		 */		
		public var currentAssetDuration:int = 0;
		...
		public function initAdManager():void{
			...
			if (this.adManagerLoaded && (this.entryReady || configProxy.vo.flashvars.sourceType != SourceType.ENTRY_ID)){
				if (!this.am){
		...
		private function configAm():void{
			...
			//if we have entry object - take duration from it, otherwise, take duration set externally
			var duration:int = (this.mediaProxy.vo.entry.duration && this.mediaProxy.vo.entry.duration != int.MIN_VALUE) ? this.mediaProxy.vo.entry.duration : currentAssetDuration;
		...
		public function set visitorId(v:String):void{
			this.parameters.uiConf[FreeWheelParameters.PARAM_CUSTOM_VISITOR] = v;
		}
