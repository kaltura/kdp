package com.kaltura.kdpfl.plugin.component
{
	...
	public class DoubleclickMediator extends SequenceMultiMediator
	{
		...
		override public function handleNotification(note:INotification):void {
			...
			if(_plugin.wasAdSkipped){
				if(_prevEvent == NotificationType.DO_PLAY && note.getName()	== NotificationType.MEDIA_LOADED){
					facade.sendNotification(NotificationType.DO_PAUSE);
					_prevEvent	= note.getName();
					_plugin.wasAdSkipped	= false;
					return;
				}
			}
			...
			switch (note.getName()) {
				...
				case NotificationType.DO_PAUSE:
					eventDispatcher.dispatchEvent(new Event(NotificationType.DO_PAUSE));
					break;
