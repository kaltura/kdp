	...
	public class OverlayMediator extends Mediator {
		...
		override public function handleNotification(notification:INotification):void {
			...
			switch (notification.getName()) {
				...
				case "changeOverlayDisplayDuration":
					var newDuration:Number = notification.getBody().newDuration; 
		...
		public function updateBannerLocation(width:Number, height:Number):void {
			// if is showing, set height as showing
			if (_overlayIsShowing) {
				overlayMC.y = height - overlayMC.height;
			}
			else {
				overlayMC.y = height;
			}
			overlayMC.x = (width - overlayMC.width) / 2;
		}
		...
		public function fireTrackingEvent(eventType:String):void {
			var currentOverlayConfig:Object = (viewComponent as overlayPluginCode).currentOverlayConfig; 
			if (currentOverlayConfig && currentOverlayConfig.trackingEvents[eventType]) {
				for (var i:int = 0; i < currentOverlayConfig.trackingEvents[eventType].length; i++) {
					var beacon:Beacon = new Beacon(currentOverlayConfig.trackingEvents[eventType][i], new HTTPLoader());
					beacon.ping();
				}
			}
		}
		...
		/**
		 * fire impressions for overlay 
		 * 
		 */		
		private function fireImpressions():void {
			var currentOverlayConfig:Object = (viewComponent as overlayPluginCode).currentOverlayConfig; 
			if (currentOverlayConfig && currentOverlayConfig.impressions) {
				for (var i:int = 0; i < currentOverlayConfig.impressions.length; i++) {
					var beacon:Beacon = new Beacon(currentOverlayConfig.impressions[i], new HTTPLoader());
					beacon.ping();
				}
			}
		}
...
//			}
		...
		private function bannerLoaded(e:Event):void {
			if (e.type == Event.COMPLETE) {
				...
				// create a click event listener for the banner
		...
		/**
		 * Function starts counting the time when the banner
		 *  is lowered until the banner is raised again.
		 *
		 */
		...
		/**
		 * Function starts the timer which counts the duration the banner is displayed
		 *  (between the banner being raised and being lowered)
		 *
		 */
		...
		/**
		 * When the timer counting the interval between overlays reaches its 
		 * complete time, this function raises the banner to be visible and 
		 * starts the timer which counts the duration of the banner being shown.
		 * @param e
		 *
		 */
		private function raiseBanner(e:TimerEvent = null):void {
			var currentOverlayConfig:Object = (viewComponent as overlayPluginCode).currentOverlayConfig;
			if ( !currentOverlayConfig || !currentOverlayConfig["nonLinearResource"] )
			{
				return;
			}
			loadBanner(currentOverlayConfig["nonLinearResource"]);
		...
		/**
		 * When the timer counting the duration of the banner being displayed reaches 
		 * its complete time, this function lowers the banner so that it is no longer 
		 * visible and starts the timer which counts the duration of the interval 
		 * between the banners being shown.
		 * @param e
		 *
		 */
		...
		/**
		 * Function forcibly stops the timer that counts the interval 
		 * time between two banners being shown.
		 *
		 */
		...
		/**
		 * Function forcibly stops the timer counting the 
		 * duration of the banner being displayed.
		 *
		 */
