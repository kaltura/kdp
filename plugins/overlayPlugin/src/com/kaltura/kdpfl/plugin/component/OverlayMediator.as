package com.kaltura.kdpfl.plugin.component {
	...
	public class OverlayMediator extends Mediator {
		 ...
		 */
		public var overlayConfig:Object;
		 ...
		 */
		public function OverlayMediator(newOverlayConfig:Object, mediatorName:String = null, viewComponent:Object = null) {
			super(NAME, viewComponent);
			overlayMC = viewComponent as OverlayMC;
		...
		override public function listNotificationInterests():Array {
			var notes:Array = ["mediaReady", "playerUpdatePlayhead", "changeMedia", "playerPlayEnd", "doPlay", "doPause"];
			return notes;
		}
		...
		override public function handleNotification(notification:INotification):void {
			...
			switch (notification.getName()) {
				case "playerUpdatePlayhead":
					...
					if (overlayConfig.currentOverlayParams) {
						if (!_overlayIsShowing) {
							if (currentTime >= overlayConfig.overlayStartAt && !_shownFirstOverlay) {
				...
				case "doPlay":
					if (sequenceProxy["vo"]["isInSequence"])
						return;
					if (!firedStart && overlayConfig.currentOverlayParams) {
						firedStart = true;
		...
		public function fireTrackingEvent(eventType:String):void {
			if (overlayConfig.currentOverlayParams && overlayConfig.currentOverlayParams.trackingEvents[eventType]) {
				for (var i:int = 0; i < overlayConfig.currentOverlayParams.trackingEvents[eventType].length; i++) {
					var beacon:Beacon = new Beacon(overlayConfig.currentOverlayParams.trackingEvents[eventType][i], new HTTPLoader());
					beacon.ping();
				}
			}
		}
		...
		private function onBannerClick(e:MouseEvent):void {
			var urlReq:URLRequest = new URLRequest(overlayConfig.currentOverlayParams["nonLinearClickThrough"]);
		...
		private function startOverlayIntervalTimer():void {
			if (!_overlayIntervalTimer) {
				_overlayIntervalTimer = new Timer(overlayConfig.overlayInterval * 1000, 1);
			}
		...
		private function startOverlayDurationTimer():void {
			if (!_showOverlayTimer) {
				_showOverlayTimer = new Timer(overlayConfig.overlayDisplayDuration * 1000, 1);
			}
		...
		private function raiseBanner(e:TimerEvent = null):void {
			loadBanner(overlayConfig.currentOverlayParams["nonLinearResource"]);
			_overlayIsShowing = true;
