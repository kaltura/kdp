package com.kaltura.kdpfl.plugin.component
{
	...
	public class VastCompanionAdProxy
	{
				...
				for each (companionConfig in flashCompsConfig)
				{
					for each (var companion : VAST2CompanionElement in companionAdsArr)
					{
						if (companionConfig.compWidth == companion.width && companionConfig.compHeight == companion.height)
						{
							...
							if (compElement.resourceType == VASTAdResourceType.STATIC || compElement.staticResource)
				...
				for each (companionConfig in flashCompsConfig)
				{
					for each (var comp : VASTCompanionAd in companionAdsArr)
					{
						if (companionConfig.compWidth == comp.width && companionConfig.compHeight == comp.height)
						{
							...
							if(companionElement.resourceType == VASTAdResourceType.STATIC)
							{
								flashCompanionMap[companionConfig.compId] = {companionAd:companionElement, relativeTo:companionConfig.compRelativity};
								//Remove this item from the array so it will not come up again for html companion ad map creation
								
							}
							else if (companionElement.resourceType == VASTAdResourceType.IFRAME)
							{
								companionElement = correctCompanion(companionElement);
								if (companionElement.resourceType == VASTAdResourceType.STATIC)
								{
									flashCompanionMap[companionConfig.compId] = {companionAd:companionElement, relativeTo:companionConfig.compRelativity};
								}
		...
		/**
		 * Function meant SPECIFICALLY for cases where the ad-server does not return the companion ad tag correctly in the VAST XML, but returns it as a url to decode.
		 * @param companion - UniformCompanionAd to correct.
		 * @return the fixed companion ad object.
		 * 
		 */		
		private function correctCompanion (companion : UniformCompanionAd ) : UniformCompanionAd 
		{
			var companionUrl : String = companion.companionResource;
			companionUrl = unescape(companionUrl);
			var companionVarsString : String = companionUrl.slice(companionUrl.indexOf("?") + 1);
			var companionVars : Array = companionVarsString.split("&");
			var companionVarsObj : Object = new Object();
			var keyValuePair : Array;
			for (var i:int =0; i < companionVars.length; i++)
			{
				keyValuePair = (companionVars[i] as String).split("=");
				companionVarsObj[keyValuePair[0]] = keyValuePair[1];
			}
			if (companionVarsObj["COMPANION_RESOURCE_TYPE"] == "STATIC")
			{
				companion.resourceType = VASTAdResourceType.STATIC;
				companion.companionResource = companionVarsObj["COMPANION_STATIC_URL"];
				companion.companionClickThrough = companionVarsObj["COMPANION_CLICK_THROUGH"]
			}
			return companion;
		}
